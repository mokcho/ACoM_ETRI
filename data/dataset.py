...
class UrbanSoundDataset(Dataset):
    def __init__(self, root_dir, annotation_dir, sample_rate=16000, test_fold=1, train=True):
        self.root_dir = root_dir
        full_df = pd.read_csv(annotation_dir)
        self.sample_rate = sample_rate

        if train:
            self.df = full_df[full_df['fold'] != test_fold].reset_index(drop=True)
        else:
            self.df = full_df[full_df['fold'] == test_fold].reset_index(drop=True)

        self.label2id = {label: i for i, label in enumerate(sorted(full_df['class'].unique()))}
        self.id2label = {idx: label for label, idx in self.label2id.items()}
    ...
    def __getitem__(self, idx):
        ...
        file_path = os.path.join(self.root_dir, f"fold{row['fold']}", row['slice_file_name'])
