...
from models.CRNN import CRNN, CRNN_respost
...
def train_model(train_loader, model, optimizer, c_epoch,ema_model=None, mask_weak=None, mask_strong=None, adjust_lr=False):
    ...
    for i, ((batch_input), target) in enumerate(train_loader):  # Ignore EMA input
        ...
        # for name, param in model.post_filter.named_parameters():
        #     if param.grad is not None:
        #         print(f"{name}: grad mean {param.grad.mean().item():.4e}, std {param.grad.std().item():.4e}")
        #     else:
...
if __name__ == '__main__':
    ...
    # Model
    # ##############
    crnn = CRNN_respost(**crnn_kwargs)

    model_path = '/home/work/jjk/data/dcase_ckpt/dcase20_baseline_model.p'
    expe_state = torch.load(model_path, map_location="cpu")

    # 3. Load model state dict (CNN/RNN/Dense only), allow missing keys for post_filter
    print(expe_state["model"].keys())
    crnn.load_state_dict(expe_state["model"]["state_dict"], strict=False)


    # /home/work/jjk/data/audioset/audioset/metadata/validation/validation.tsv
    # /home/work/jjk/data/audioset/audioset/audio/validation
    #   None

    val_dataset = DESED(base_feature_dir=os.path.join(cfg.workspace, "dataset", "features"), compute_log=False, recompute_features=False, use_encodec=True, bitrate=f_args.bitrate)


    gt_df_feat = val_dataset.initialize_and_get_df('/home/work/jjk/data/audioset/audioset/metadata/validation/validation.tsv', '/home/work/jjk/data/audioset/audioset/audio/validation', nb_files=None)
    ...
    for epoch in range(50):
        ...
        # predictions = get_predictions(crnn, params["dataloader"], many_hot_encoder.decode_strong,
        #                                 pooling_time_ratio, median_window=median_window,
        #                                 save_predictions=None)
        valid_predictions = get_predictions(crnn, params["dataloader"], many_hot_encoder.decode_strong,
                                        pooling_time_ratio, median_window=median_window,
                                        save_predictions=None)
        valid_f1, psds_m_f1 = compute_metrics(valid_predictions, groundtruth, durations)
        ...
        # valid_f1, psds_m_f1 = compute_metrics(predictions, valid, durations_synth)
        ...
        if cfg.save_best:
            if save_best_cb.apply(valid_f1):
                model_fname = os.path.join(saved_model_dir, "baseline_best")
                torch.save(state, model_fname)
            results.loc[epoch, "global_valid"] = valid_f1
        results.loc[epoch, "loss"] = loss_value.item()
        results.loc[epoch, "valid_f1"] = valid_f1

        if cfg.early_stopping:
            if early_stopping_call.apply(valid_f1):
    ...
    model_fname = os.path.join(saved_model_dir, "finetuned")
    torch.save(state, model_fname)
    ...
    compute_metrics(valid_predictions, groundtruth, durations)
