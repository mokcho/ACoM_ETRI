...
class DataLoadDf(Dataset):
    ...
    def __init__(self, df, encode_function=None, transform=None, return_indexes=False, in_memory=False):
        ...
        self.x_fallback_shape = None
        self.y_fallback_shape = None
    ...
    def __getitem__(self, index):
        ...
        if sample is None:
            if self.x_fallback_shape is not None and self.y_fallback_shape is not None:
                sample = (torch.zeros(self.x_fallback_shape), torch.zeros(self.y_fallback_shape))
            else:
                raise ValueError("Fallback shapes are not defined, but sample is None.")
        ...
        if isinstance(sample, tuple) and len(sample) == 2:
            ...
            raise ValueError(f"Unexpected sample structure: {type(sample)} {sample}")

        # If x is still a tuple, take its first element
        if isinstance(x, tuple):
            x = x[0]

        if self.x_fallback_shape is None:
            self.x_fallback_shape = x.shape
            self.y_fallback_shape = y.shape
