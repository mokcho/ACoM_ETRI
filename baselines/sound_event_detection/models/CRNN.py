...
from models.post_encodec import *
...
class CRNN(nn.Module):
    ...
    def __init__(self, n_in_channel, nclass, attention=False, activation="Relu", dropout=0,
        ...
        self.n_in_cnn = n_in_channel
        if cnn_integration:
            self.n_in_cnn = 1
        self.cnn = CNN(self.n_in_cnn, activation, dropout, **kwargs)
        if not train_cnn:
...
class CRNN_respost(CRNN):
    def __init__(self, n_in_channel, nclass, attention=False, activation="Relu", dropout=0,
                 train_cnn=True, rnn_type='BGRU', n_RNN_cell=64, n_layers_RNN=1, dropout_recurrent=0,
                 cnn_integration=False, **kwargs):

        super().__init__(n_in_channel, nclass, attention, activation, dropout,
                         train_cnn, rnn_type, n_RNN_cell, n_layers_RNN, dropout_recurrent,
                         cnn_integration, **kwargs)

        self.post_filter = PostEncodecEnhancer2D(in_channels=self.n_in_cnn)

        for param in self.parameters():
            param.requires_grad = False

        for param in self.post_filter.parameters():
            param.requires_grad = True

    def forward(self, x):
        # Apply 2D post-filter before CNN
...
if __name__ == '__main__':
    CRNN_respost(64, 10, kernel_size=[3, 3, 3], padding=[1, 1, 1], stride=[1, 1, 1], nb_filters=[64, 64, 64],
         pooling=[(1, 4), (1, 4), (1, 4)])
