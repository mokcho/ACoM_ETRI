...
# Scoring helper
# ------------------------------
def _compute_score(processor, wav_compressed, y_true_clip_i):
    """Shared scoring logic for all pruning modes."""
    with torch.no_grad():
        output = processor.pipeline(wav_compressed, padding_mask=None)
        logits = output[0] if isinstance(output, tuple) else output

        if processor.dataset_type in ('esc50', 'urbansound8k'):
            return logits[0, y_true_clip_i].item()
        elif processor.dataset_type in ('audioset'):
            true_classes = y_true_clip_i.nonzero(as_tuple=True)[0]
            false_classes = (y_true_clip_i == 0).nonzero(as_tuple=True)[0]

            if len(true_classes) > 0 and len(false_classes) > 0:
                return (logits[0, true_classes].mean() - logits[0, false_classes].mean()).item()
            elif len(true_classes) > 0:
                return logits[0, true_classes].mean().item()
            else:
                return logits[0].max().item()
        else:
            raise NotImplementedError


# ------------------------------
# Importance map selection
# ------------------------------
def _get_importance(mode, S_orig_i, sal_map_i, importance_source):
    """
    Returns a 1D importance vector for the given pruning mode.
    
    importance_source:
        'saliency' — model-derived saliency map (BEATs activations / AST attention)
        'energy'   — spectral magnitude |S|, no model needed
        'random'   — uniform random (ablation baseline)
    """
    F_bins, T_bins = S_orig_i.shape
    
    if importance_source == 'energy':
        imp_2d = S_orig_i.abs()                          # (F, T)
    elif importance_source == 'random':
        imp_2d = torch.rand(F_bins, T_bins, device=S_orig_i.device)
    else:  # 'saliency'
        imp_2d = sal_map_i                               # (F, T)
    
    if mode == 'freq':
        importance = imp_2d.mean(dim=1)                  # (F,)
    elif mode == 'time':
        importance = imp_2d.mean(dim=0)                  # (T,)
    elif mode == 'pixel':
        importance = imp_2d.reshape(-1)                  # (F*T,)
    else:
...
# ------------------------------
# Adaptive threshold masking (replaces oracle sweep)
# ------------------------------
def _threshold_mask(mode, S_orig_i, sal_map_i, processor, compress_fn,
                    kbps, length_i, y_true_clip_i, alpha, importance_source):
    """
    Single-pass adaptive masking:  τ = mean - α * std
    
    Everything below τ in the importance signal gets zeroed before codec roundtrip.
    No sweep — one mask, one codec pass, done.
    
    Returns (masked_ratio, score, wav_out, axis_size).
    """
    F_bins, T_bins = S_orig_i.shape
    
    importance = _get_importance(mode, S_orig_i, sal_map_i, importance_source)
    axis_size = importance.numel()
    
    # ---- adaptive threshold: τ = mean - α * std ----
    mu = importance.mean()
    sigma = importance.std()
    tau = mu - alpha * sigma
    
    # build binary mask (1 = keep, 0 = remove)
    keep = (importance >= tau).float()
    n_masked = int((keep == 0).sum().item())
    
    # reshape mask to match spectrogram
    if mode == 'freq':
        mask = keep.unsqueeze(1)                         # (F, 1)
    elif mode == 'time':
        mask = keep.unsqueeze(0)                         # (1, T)
    elif mode == 'pixel':
        mask = keep.reshape(F_bins, T_bins)              # (F, T)
    
    S_masked = S_orig_i * mask
    
    # reconstruct → compress → score
    wav = processor.istft(S_masked.unsqueeze(0), length=length_i)
    wav_compressed = compress_fn(wav, kbps=kbps)
    score = _compute_score(processor, wav_compressed, y_true_clip_i)
    
    masked_ratio = n_masked / axis_size
    wav_out = wav_compressed.squeeze().cpu()
    
    return masked_ratio, score, wav_out, axis_size, n_masked


# ------------------------------
# Processing function (multi-mode)
# ------------------------------
def process_fold_kbps(processor, saliency_filter, output_dirs, fold_or_split,
                      kbps, codec='encodec', pruning_modes=None,
                      alpha=0.5, importance_source='saliency'):
    """
    output_dirs:        dict  {mode: dir_path}
    pruning_modes:      list  e.g. ['freq', 'time', 'pixel']
    alpha:              float threshold aggressiveness  (τ = mean - α * std)
    importance_source:  str   'saliency' | 'energy' | 'random'
    
    Saliency is computed ONCE per batch (skipped entirely for energy/random).
    """
    if pruning_modes is None:
        pruning_modes = ['freq']
    
    results = {mode: [] for mode in pruning_modes}
    
    mode_str = '+'.join(pruning_modes)
    logging.info(f"Processing {fold_or_split} @ {kbps} kbps with {codec.upper()} "
                 f"[{processor.model_type.upper()}] modes={mode_str} "
                 f"α={alpha} importance={importance_source}")
    
    pbar = tqdm(processor.dataloader,
                desc=f"{fold_or_split} @ {kbps}kbps ({codec}/{mode_str}/α={alpha})")
    ...
    for batch_data in pbar:
        ...
        elif processor.dataset_type in ('esc50', 'urbansound8k'):
            y_true_clip = y_true
            if isinstance(batch_data[2], list):
        ...
        # ========== IMPORTANCE SOURCE ==========
        # For 'saliency': run model forward to get saliency maps
        # For 'energy' or 'random': still need S_orig from STFT, but skip model
        if importance_source == 'saliency':
            ...
            sal_map = None  # not needed for energy/random
        ...
        for i in range(B):
            ...
            for mode in pruning_modes:
                save_path = os.path.join(output_dirs[mode], filename)
                if os.path.exists(save_path):
                    continue
                
                sal_i = sal_map[i] if sal_map is not None else None
                
                masked_ratio, score, wav_out, axis_size, n_masked = _threshold_mask(
                    mode=mode,
                    S_orig_i=S_orig[i],
                    sal_map_i=sal_i,
                    processor=processor,
                    ...
                    kbps=kbps,
                    length_i=lengths[i],
                    y_true_clip_i=y_true_clip[i],
                    alpha=alpha,
                    importance_source=importance_source,
                )
                
                torchaudio.save(save_path, wav_out.unsqueeze(0), processor.sr)
                ...
                results[mode].append({
                    ...
                    'masked_units': n_masked,
                    'total_units': axis_size,
                    'score': score,
    ...
    # Flatten results across modes
    all_results = []
    for mode in pruning_modes:
        all_results.extend(results[mode])
    return all_results
...
def main():
    ...
    ap.add_argument("--pruning_modes", nargs="+", type=str, default=['freq'],
                     choices=['freq', 'time', 'pixel'],
                     help="Masking axes to run (all share one saliency pass)")
    ap.add_argument("--alpha", nargs="+", type=float, default=[0.5],
                     help="Threshold aggressiveness: τ = mean - α*std. "
                          "Multiple values will be swept sequentially.")
    ap.add_argument("--importance", type=str, default='saliency',
                     choices=['saliency', 'energy', 'random'],
                     help="Importance signal: saliency (model-derived), "
                          "energy (spectral magnitude), random (ablation)")
    args = ap.parse_args()
    ...
    mode_str = '+'.join(args.pruning_modes)
    alpha_str = ','.join(f'{a:.2f}' for a in args.alpha)
    logging.info(f"Model: {model_type.upper()} | Codec: {args.codec.upper()} | "
                 f"Modes: {mode_str} | α: [{alpha_str}] | Importance: {args.importance}")
    
    all_results = []
    
    def _make_output_dirs(base, codec, importance, alpha_val, modes, *sub_parts):
        """Create and return {mode: path} dict, namespaced by importance source + alpha."""
        dirs = {}
        for m in modes:
            d = os.path.join(base, codec, importance, f"alpha_{alpha_val:.2f}", m, *sub_parts)
    ...
    if 'ESC-50' in cfg.data_dir:
        ...
        for fold in folds:
            ...
            for alpha_val in args.alpha:
                for kbps in args.kbps_list:
                    output_dirs = _make_output_dirs(
                        args.output_dir, args.codec, args.importance, alpha_val,
                        args.pruning_modes, f"fold_{fold}", f"kbps_{kbps}"
                    )
                    ...
                    fold_results = process_fold_kbps(
                        ...
                        kbps=kbps,
                        codec=args.codec,
                        pruning_modes=args.pruning_modes,
                        alpha=alpha_val,
                        importance_source=args.importance,
                    )
        ...
        for alpha_val in args.alpha:
            for kbps in args.kbps_list:
                output_dirs = _make_output_dirs(
                    args.output_dir, args.codec, args.importance, alpha_val,
                    args.pruning_modes, f"{kbps}kbps"
                )
    ...
    # Append if file exists (allows parallel runs)
    if os.path.exists(csv_path):
        existing = pd.read_csv(csv_path)
        df = pd.concat([existing, df], ignore_index=True).drop_duplicates(
            subset=['filename', 'fold_or_split', 'kbps', 'codec', 'pruning_mode',
                    'importance_source', 'alpha'],
            keep='last'
        )
    ...
    logging.info(f"{'='*50}")
    logging.info(f"Model: {model_type.upper()} | Codec: {args.codec.upper()} | "
                 f"Modes: {mode_str} | Importance: {args.importance}")
    logging.info(f"Saved {len(all_results)} masked audio files")
    logging.info(f"Saved results to {csv_path}")
    
    logging.info(f"\nPer-mode / per-alpha summary:")
    for alpha_val in args.alpha:
        alpha_df = df[df['alpha'] == alpha_val]
        for mode in args.pruning_modes:
            mode_df = alpha_df[alpha_df['pruning_mode'] == mode]
            if len(mode_df) > 0:
                logging.info(f"  [{args.importance}] α={alpha_val:.2f} [{mode}] "
                            f"ratio={mode_df['masked_ratio'].mean():.3f}, "
                            f"score={mode_df['score'].mean():.3f}, n={len(mode_df)}")
                for split in mode_df['fold_or_split'].unique():
                    split_df = mode_df[mode_df['fold_or_split'] == split]
                    for kbps in args.kbps_list:
                        kbps_df = split_df[split_df['kbps'] == kbps]
                        if len(kbps_df) > 0:
                            logging.info(f"    {split} @ {kbps}kbps: "
                                        f"ratio={kbps_df['masked_ratio'].mean():.3f}, "
                                        f"score={kbps_df['score'].mean():.3f}")
